...
osascript -e 'tell application "Terminal" to do script "cd CHANGE_TO_YOUR_PARENT_DIRECTORY_TO_COOKER/cooker && source venv/bin/activate" in front window'
...
osascript -e 'tell application "System Events" to tell process "Terminal" to keystroke "t" using command down'
osascript -e 'tell application "Terminal" to do script "cd CHANGE_TO_YOUR_PARENT_DIRECTORY_TO_COOKER/cooker && npm run watch:css" in front window'

# Building your tailwind.min.css file
osascript -e 'tell application "System Events" to tell process "Terminal" to keystroke "t" using command down'
osascript -e 'tell application "Terminal" to do script "cd CHANGE_TO_YOUR_PARENT_DIRECTORY_TO_COOKER/cooker && npm run build:css" in front window'

# Open up VS code Text and the tab used to run flask-commands commands (no virtual enviroment)
osascript -e 'tell application "System Events" to tell process "Terminal" to keystroke "t" using command down'
osascript -e 'tell application "Terminal" to do script "cd CHANGE_TO_YOUR_PARENT_DIRECTORY_TO_COOKER/cooker && code ." in front window'
